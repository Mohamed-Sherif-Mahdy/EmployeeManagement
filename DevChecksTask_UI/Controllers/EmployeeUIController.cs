    ...
    public class EmployeeUIController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var ListOfDtoEmployees = JsonConvert.DeserializeObject<
                    List<EmployeeWithJobTitleDto>
                >(data);
        ...
        public IActionResult Create(EmployeeWithJobTitleDto DtoEmployee)
        {
            ...
            HttpResponseMessage response = _client
                .PostAsync("EmployeeManagementAPI", content)
                .Result;
            if (response.IsSuccessStatusCode)
            {
        ...
        public IActionResult Edit(int id, EmployeeWithJobTitleDto DtoEmployee)
        {
            ...
            var content = new StringContent(data, Encoding.UTF8, "application/json");
            HttpResponseMessage response = _client
                .PutAsync($"EmployeeManagementAPI/{id}", content)
                .Result;
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
        ...
        {
            HttpResponseMessage response = _client
                .DeleteAsync($"EmployeeManagementAPI/{id}")
                .Result;
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
